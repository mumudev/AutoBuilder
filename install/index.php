...
<html>
	<head>
		...
		<title>MumuDev 安装向导 </title>
	</head>
	<body>
		<div id="warp">
		<h1 style='line-height:40px'><img src='images/logo.png' width='35' height='35'/>MumuDev安装程序</h1>
			<?php
				if ( !$step ){
			?>
			<script type="text/javascript">
			...
			<div id="content">
				...
				<ul class="l1 nt">
					<h3>MumuDev介绍</h3>
					<li>MumuDev 是由 Mumu 独立开发的博客系统，亦可作为CMS使用，基于PHP脚本和MySQL数据库。本程序源码开放的，任何人都可以从互联网上免费下载，并可以在不违反本协议规定的前提下进行使用而无需缴纳程序使用费。</li>
					<li>官方网址： www.mumuimp.com</li>
					<li>为了使你正确并合法的使用本软件，请你在使用前务必阅读清楚下面的协议条款：</li>
					<li>mumu(mumuimp.com) 为 MumuDev 的作者，依法独立拥有MumuDev产品著作权。</li>
	  				<li>用者：无论个人或组织、盈利与否、用途如何（包括以学习和研究为目的），均需仔细阅读本协议，在理解、同意、并遵守本协议的全部条款后，方可开始使用MumuDev。</li>
	  				<li>mumuimp.com 有对本授权协议的最终解释权。</li>
  				</ul>
			  	<ul class="l1 nt">
			  		<h3>ONE--协议许可的权利</h3>
			  		<li>您可以在完全遵守本最终用户授权协议的基础上，将本软件应用于非商业用途，而不必支付软件版权授权费用；</li>
			  		<li>您可以在协议规定的约束和限制范围内修改 MumuDev 源代码或界面风格以适应您的网站要求；</li>
			  		<li>您拥有使用本软件构建的网站中全部资料、文章及相关信息的所有权，并独立承担与文章内容的相关法律义务；</li>
			  	</ul>
				<ul class="l1 nt">
  					<h3>TWO--协议规定的约束和限制</h3>
  					<li>不得对本软件进行出租、出售、抵押或发放子许可证；</li>
  					<li>无论如何，即无论用途如何、是否经过修改或美化、修改程度如何，只要使用MumuDev的整体或任何部分，未经书面许可，页面页脚处的 Powered by MumuDev和官网网站的链接（http://www.mumuimp.com ）都必须保留，而不能清除或修改；</li>
  				...
  				<ul class="l1 nt">
  					...
  					<li>mumu(mumuimp.com)不对使用本软件构建的网站中的文章或信息承担责任。</li>
  				</ul>
				<p class="nt" style="padding-bottom:20px;">电子文本形式的授权协议如同双方书面签署的协议一样，具有完全的和等同的法律效力。您一旦开始安装 MumuDev 即被视为完全理解并接受本协议的各项条款，在享有上述条款授予的权力的同时，受到相关的约束和限制。协议许可范围以外的行为，将直接违反本授权协议并构成侵权，我们有权随时终止授权，责令停止损害，并保留追究相关责任的权力。</p>
			</div>
			<div class="ch_next">
			...
			<div id="content">
				...
				<dl class="l1 nt wrbg">
					<dt>数据库名：</dt>
					<dd></dd>
					<dd><input type="text" name="db_name" value="MumuDev" size="40" class='input' /></dd>
				</dl>
				<dl class="l1 nt">
					<dt>表名前缀：</dt>
					<dd></dd>
				...
				<dl class="l1 nt">
					<dt>密码：</dt>
					<dd>默认密码为admin,为确保您网站管理的安全，密码建议大于6位数，并且是字母+数字或者符号</dd>
					<dd><input type="password" name="password" value="admin" size="40" class='input' /></dd>
				</dl>
				<dl class="l1 nt wrbg">
					<dt>重复密码：</dt>
					<dd></dd>
					<dd><input type="password" name="rpassword" value="admin" size="40" class='input' /></dd>
				</dl>
				<dl class="l1 nt wrbg">
					<dt>电子邮件：</dt>
					<dd></dd>
					<dd><input type="text" name="email" value="mumudev@163.com" size="40" class='input' /></dd>
				</dl>
			</div>
			<div class="ch_next">
...
EOT;
				...
				<?php
					...
					if (!$db_config['db_host'] && !$db_config['db_name']){
						...
						if($sqlv < '4.1'){
							...
							if( intval($link[0]) > 0 ){
								$rebuild = true;
								$message .= '<p class="message">数据库中已经安装过 MumuDev，继续安装会清空原有数据！</p>';
								$alert = ' onclick="return confirm(\'数据库中已经安装过 MumuDev，继续安装会清空原有数据！\');"';
							}
						}
					}
					if ( $quit ){
						...
						if ( $rebuild ){
							echo '<p class="message" style="color:red;font-size:16px;">数据库中已经安装过 MumuDev，继续安装会清空原有数据！</p>';
						}
			...
			<div id="content" style="height:448px;">
				...
				<dl class="l1 nt">
					...
					<dd><a href="../index.php">开始体验MumuDev为你带来的乐趣吧！</a></dd>
